...
void Update(Heap heap[300009], long long int v, long long int dist)
{
	long long int i=pos[v];
	heap[i].dist=dist;
	while (heap[i].dist<heap[(i-1)>>1].dist && i>0)
		i=compute(i);
}
...
long long int Present( long long int k,long long int pos[])
{
	if (size>pos[k])
		return 1;
	return 0;
...
void dijkstra(long long int src,long long int arr[300009],long long int p[])
{
	...
	while (size>0)
	{
		...
		while (temp!=NULL)
		{
			...
			if (Present(v,pos)==1 && arr[u]!=INT_MAX && 
					temp->wt+arr[u]<arr[v])
			{
				p[v] = p[u];
				arr[v]=arr[u]+temp->wt;
			...
			else if (Present(v,pos)==1 && arr[u]!=INT_MAX && temp->wt+arr[u]>arr[v])
				p[u] = p[v];
